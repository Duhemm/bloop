...
class MultiplexedStreamsSpec {
  ...
  def systemOutAndErrAreHijacked() = {
    ...
    assertTrue(s"${messages.mkString("\n")} didn't contain $stdoutNeedle", messages.contains(stdoutNeedle))
    assertTrue(s"${messages.mkString("\n")} didn't contain $stderrNeedle", messages.contains(stderrNeedle))
  }
  ...
  def concurrentlyRunningProcessesHaveDifferentOutputs = {
    ...
    assertTrue(s"${messagesT0.mkString("\n")} were not all $expectedT0", messagesT0.forall(_ == expectedT0))
    assertTrue(s"${messagesT1.mkString("\n")} were not all $expectedT1", messagesT1.forall(_ == expectedT1))
  }
